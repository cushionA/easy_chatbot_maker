...
Ticket（タイトル + 本文 + 優先度 + `tenantId` + `knowledgeEntryId`）を、テナント設定の起票先（Redmine / GitHub Issues）へ送信できる状態にする。API キーは **Secret Manager から取得して使う**（アプリ設定ファイルに持たせない）。
...
- `testConnection` で接続テストでき、`submit` で実起票できる（リトライ + 認証エラー即失敗）
- 起票時の API キーは Secret Manager クライアント（IAM 権限）で取得する
- destination の登録/編集 + 接続テストボタンの最小 React UI + Node API が動く
...
| 日 | テーマ | 主成果物 |
|---|---|---|
| Day 1 | 抽象を確定する | 詳細: [`sprint3/day1.md`](sprint3/day1.md) — 新規 `apps/api/src/destinations/` モジュール、`ITicketDestination` / `DestinationConfig` / `Ticket` / `TicketSubmitResult` / `TestConnectionResult` TypeScript interface/型、`DestinationRegistry` + DI/ファクトリ、起票本文 Markdown 化（`build_description` 移植） |
| Day 2 | アダプタを実装する | 詳細: [`sprint3/day2.md`](sprint3/day2.md) — `RedmineDestination`（最初の 1 個）、`GitHubIssuesDestination`（同パターン複製）、`testConnection`、リトライ（指数バックオフ最大 3 回）+ 認証エラー即失敗、アダプタ単体テスト |
| Day 3 | Secret Manager と UI を結線する | 詳細: [`sprint3/day3.md`](sprint3/day3.md) — Secret Manager でのシークレット作成/取得、`IDestinationSecretStore` TS interface と実装、destination 登録/編集 + 接続テスト最小 React + Node API、起票失敗時 `draft_fields` 保持・成功時 NULL クリア、フィールドマッピング（JSONB）の優先度変換 |
...
> 「インターフェースと型を自分の手で握る」日。設計書 06 章のシグネチャをそのまま `apps/api/src/destinations/` モジュールに落とす。実装はまだしない。本文 Markdown 化（PoC 資産の移植）だけ AI に投げる。

- **Day1-1.** `apps/api/src/destinations/` モジュールを作成 [自分]
- **Day1-2.** `ITicketDestination` interface + 型 4 つを定義 [自分（中核）]
- **Day1-3.** `DestinationRegistry`（kind → 実装解決）+ DI/ファクトリ登録 [自分]
- **Day1-4.** 起票本文 Markdown 化（既存 Streamlit `build_description` の TypeScript 移植）[AI]
...
- **Day2-4.** アダプタ単体テスト（msw/nock でモック HTTP、リトライ/即失敗を検証）[AI 一次実装 → 自分レビュー]

---

## Day 3 — Secret Manager と UI を結線する

> 「秘匿情報の境界」を自分の手で握る日。Secret Manager クライアントのラッパを自分で書き、登録 UI と起票失敗ハンドリングを AI に複製させる。

- **Day3-1.** Secret Manager でのシークレット作成/取得手順を確立する [自分]
- **Day3-2.** `IDestinationSecretStore`（アプリ層から Secret Manager を呼ぶ薄いラッパ）[自分]
- **Day3-3.** destination 登録/編集 + 接続テストボタンの最小 React + Node API [AI]
...
3. インターフェース・型・Secret Manager 手順の雛形を自分で書く（[自分] タスクの中身）
...
| Secret Manager の IAM 権限エラー | [`04_security_multitenant.md:121-142`](04_security_multitenant.md)（シークレット管理） |
...
- **起票画面の本結線**（分類確定 → 動的フォーム入力 → `Ticket` 組立 → `submit`）— Sprint 4 の主題
